...
export class HabitManagerController extends BaseController {
	...
	static #createSchema = {
		name: {required: true, type: 'string'}, freq: {required: true, type: 'string'},
	};
	...
	listHabits(_, {renderer}) {
		try {
			...
			if (!habits.length) {
				renderer.error('No habits found. Use `add` command to create one.');
				return;
			}
	...
	findHabitById(args, {renderer}) {
		...
		try {
			...
			if (!habit) {
				renderer.error(`No habit found with ID: ${args.id}`);
				return;
			}
